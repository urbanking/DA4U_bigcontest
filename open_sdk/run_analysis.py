...
def get_marketplace_json(address: str) -> Dict[str, Any]:
    """Step 6: Marketplace Analysis (Mandatory) - Keyword-based matching + Gemini fallback"""
    print("\n" + "="*60)
    print("[Step 6] Marketplace Analysis")
    print("="*60)
    ...
    try:
        ...
        if best_match and best_score > 0:
            ...
            # Gemini fallback
            print("[INFO] Keyword matching failed, using Gemini for marketplace selection...")
            try:
                from agents_new.utils.gemini_client import get_gemini_client
                gemini = get_gemini_client()
                
                file_names = [f.stem for f in json_files[:20]]
                
                prompt = f"""Address: {address}

Marketplaces: {', '.join(file_names)}

Answer with the nearest marketplace name only."""
                ...
                print(f"[INFO] Gemini selected: {response}")
                
                for json_file in json_files:
                    if json_file.stem in response or response in json_file.stem:
                        best_match = json_file
                        print(f"[OK] Gemini matching: {best_match.name}")
                        break
            except Exception as e:
        ...
        if not best_match:
            print(f"[WARN] All matching failed, using default file")
...
async def run_full_analysis_pipeline(store_code: str) -> Dict[str, Any]:
    ...
    # Step 4: Mobility analysis (Mandatory)
    mobility_result = await run_mobility_analysis(address, dong)
    
    # Step 5: Panorama analysis (Mandatory)
    panorama_result = await run_panorama_analysis(address)
    
    # Step 6: Marketplace analysis (Mandatory)
    marketplace_result = get_marketplace_json(address)
