...
with col2:
    ...
    # Google Maps 도구 섹션 추가
    if GOOGLE_MAPS_TOOLS_AVAILABLE:
        st.markdown("---")
        st.markdown("### 🗺️ Google Maps 도구")
        
        # Google Maps 도구 선택
        tool_option = st.selectbox(
            "Google Maps 도구 선택",
            ["장소 검색", "장소 상세 정보", "주소 변환", "종합 검색"],
            help="Google Maps API를 사용하여 장소 정보를 검색합니다."
        )
        
        if tool_option == "장소 검색":
            st.markdown("#### 🔍 장소 검색")
            search_query = st.text_input("검색할 장소명 또는 키워드", placeholder="예: 강남역 스타벅스, 서울 맛집")
            location = st.text_input("검색 중심 위치 (선택사항)", placeholder="예: 37.5665,126.9780")
            radius = st.slider("검색 반경 (미터)", 100, 50000, 5000)
            
            if st.button("🔍 검색", type="primary"):
                if search_query:
                    with st.spinner("장소를 검색 중..."):
                        try:
                            from agents_new.google_map_mcp.langchain_tools import search_places
                    ...
                    st.warning("검색할 장소명을 입력해주세요.")
        
        elif tool_option == "장소 상세 정보":
            st.markdown("#### 📋 장소 상세 정보")
            place_id = st.text_input("Place ID", placeholder="예: ChIJ...")
            
            if st.button("📋 상세 정보 조회", type="primary"):
                if place_id:
                    with st.spinner("상세 정보를 조회 중..."):
                        try:
                            from agents_new.google_map_mcp.langchain_tools import get_place_details
                            result = get_place_details.invoke({"place_id": place_id})
                            st.markdown(result)
                        except Exception as e:
                            st.error(f"상세 정보 조회 실패: {e}")
                else:
                    st.warning("Place ID를 입력해주세요.")
        
        elif tool_option == "주소 변환":
            st.markdown("#### 🗺️ 주소 변환")
            address = st.text_input("변환할 주소", placeholder="예: 서울특별시 강남구 테헤란로 123")
            
            if st.button("🗺️ 주소 변환", type="primary"):
                if address:
                    with st.spinner("주소를 변환 중..."):
                        try:
                            from agents_new.google_map_mcp.langchain_tools import geocode_address
                            result = geocode_address.invoke({"address": address})
                            st.markdown(result)
                        except Exception as e:
                            st.error(f"주소 변환 실패: {e}")
                else:
                    st.warning("변환할 주소를 입력해주세요.")
        
        elif tool_option == "종합 검색":
            st.markdown("#### 🔍 종합 검색")
            search_query = st.text_input("검색할 장소명 또는 키워드", placeholder="예: 강남역 스타벅스")
            location = st.text_input("검색 중심 위치 (선택사항)", placeholder="예: 37.5665,126.9780")
            radius = st.slider("검색 반경 (미터)", 100, 50000, 5000)
            
            if st.button("🔍 종합 검색", type="primary"):
                if search_query:
                    with st.spinner("종합 검색을 수행 중..."):
                        try:
                            from agents_new.google_map_mcp.langchain_tools import search_and_get_details
                            result = search_and_get_details.invoke({
                                "query": search_query,
                                "location": location if location else None,
                                "radius": radius
                            })
        ...
        st.markdown("---")
        st.markdown("### 🗺️ Google Maps 도구")
        st.warning("⚠️ Google Maps 도구를 사용할 수 없습니다. API 키를 설정해주세요.")