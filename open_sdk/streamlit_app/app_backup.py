...
try:
    from ai_agents import (
        classify_query_sync,
        create_consultation_agent,
        chat_with_consultant_sync,
        create_consultation_session,
        merge_all_analysis_results
    )
    AGENTS_AVAILABLE = True
    print("[OK] AI Agents loaded successfully")
...
if 'consultation_agent' not in st.session_state:
    st.session_state.consultation_agent = None
...
with col1:
    ...
    if prompt := st.chat_input("질문을 입력하세요..."):
        ...
        elif st.session_state.consultation_mode and AGENTS_AVAILABLE:
            # Consultation 모드: Agent로 응답
            st.session_state.messages.append({"role": "user", "content": prompt})
            
            with st.spinner("상담사가 답변을 준비중입니다..."):
                try:
                    response = chat_with_consultant_sync(
                        st.session_state.consultation_agent,
                        prompt,
                        st.session_state.consultation_session
                    )
                ...
                if st.button("💬 상담 시작", type="primary", use_container_width=True):
                    if AGENTS_AVAILABLE:
                        with st.spinner("상담 시스템을 준비중입니다..."):
                            try:
                                # 통합된 파일 로드
                                output_dir = Path(analysis_data["analysis_dir"])
                                merged_json_file = output_dir / "merged_analysis_full.json"
                                merged_md_file = output_dir / "merged_analysis_full.md"
                                
                                # 통합 파일이 있는지 확인
                                if merged_json_file.exists() and merged_md_file.exists():
                                    # JSON 파일 로드
                                    ...
                                    print(f"[DEBUG] JSON 파일: {merged_json_file}")
                                    print(f"[DEBUG] MD 파일: {merged_md_file}")
                                    print(f"[DEBUG] MD 파일 크기: {len(merged_md)} bytes")
                                    
                                    # Consultation Agent 생성 (통합 파일 데이터로 생성)
                                    ...
                                    st.error(f"통합 파일을 찾을 수 없습니다: {merged_json_file}, {merged_md_file}")
                            except Exception as e:
