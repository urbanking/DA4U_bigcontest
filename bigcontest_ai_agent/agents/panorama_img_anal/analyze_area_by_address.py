...
def address_to_coordinates(address: str) -> tuple:
    ...
    print("[INFO] Kakao API 키가 없어 Nominatim 사용 (느릴 수 있음)")
...
def encode_image_pil(pil_image: Image.Image) -> str:
...
def analyze_image_with_gpt(client: OpenAI, 
    ...
    base64_image = encode_image_pil(extracted_image)
    ...
    try:
        ...
        return {"error": str(e), "raw_response": result_text}
...
def analyze_area_by_address(address: str,
    ...
    # 8-1. 전체 분석 결과 JSON 저장
    full_json_path = f"{output_folder}/analysis_result.json"
    with open(full_json_path, 'w', encoding='utf-8') as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f"  [OK] 전체 분석 결과: {full_json_path}")
    
    # 8-2. 종합 분석만 따로 JSON 저장
    synthesis_json_path = f"{output_folder}/synthesis_only.json"
    synthesis_result = {
        "metadata": result['metadata'],
        "synthesis": synthesis
    }
    with open(synthesis_json_path, 'w', encoding='utf-8') as f:
        json.dump(synthesis_result, f, ensure_ascii=False, indent=2)
    print(f"  [OK] 종합 분석 결과: {synthesis_json_path}")
    ...
    print(f"  - analysis_result.json (전체 분석)")
    print(f"  - synthesis_only.json (종합 분석만)")
