...
class MarketplaceWrapper:
    ...
    def __init__(self):
        ...
        # 프로젝트 루트 기준 상대 경로 사용
        project_root = Path(__file__).parent.parent.parent  # bigcontest_ai_agent 디렉토리
        
        self.root = os.getenv("MARKETPLACE_ANALYSIS_ROOT", str(project_root / "agents_new" / "marketplcae_anal"))
        self.bat_path = os.getenv("MARKETPLACE_BAT_PATH", str(project_root / "agents_new" / "marketplcae_anal" / "gemini1.bat"))
        self.output_path = os.getenv("MARKETPLACE_OUTPUT_PATH", str(project_root / "agents_new" / "marketplcae_anal" / "상권분석리포트"))
